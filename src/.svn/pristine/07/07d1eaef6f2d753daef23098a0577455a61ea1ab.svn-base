    ...
    <el-dialog title="编辑" v-model="editFormVisible" :close-on-click-modal="false">
      <el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
        <el-form-item label="关联广告" prop="adId">
          <el-select class="pull-left" :disabled="editForm.adId != ''" v-model="editForm.adId" placeholder="请选择广告" @change="handleAdSelected(editForm.adId)">
            <el-option
        ...
        if (id != '') {
          ...
          for (let i = 0; i < vm.searchData.length; i++) {
            if (vm.searchData[i].adId == id) {
              ...
              vm.uploadParams.cmarketMerchantId = vm.selectedItem.merchantId
              if (vm.selectedItem.img1 != '') {
                vm.editForm.picURL = vm.previewPic = process.env.IMG_SERVER + vm.selectedItem.img1
              }
              vm.editForm.onlineTaskEnd = vm.selectedItem.onlineEnd
              break
            }
          }
        }
