...
<script>
  ...
  export default {
    ...
    methods: {
      editSubmit () {
        let vm = this
        // vm.editForm.onlineEnd =  new Date(vm.editForm.onlineStart)
        // vm.editForm.onlineEnd = 222
        vm.$refs.editForm.validate((valid) => {
          if (this.imageUrl == '') {
            ...
            if (vm.radio1 == 1 && (this.value3 == '' || this.value3 == undefined)  ) {
            vm.$message({
              message: '请先选择上下线时间',
              type: 'error'
            })
          ...
          console.log(vm.editForm.onlineEnd)
          if (valid) {
            vm.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              let params = {
                ...
                img2: vm.img2,
                onlineStart: vm.radio1 == 0 ? '' : Date.parse(vm.value3[0]),
                onlineEnd: vm.radio1 == 0 ? Date.parse(vm.editForm.onlineEnd) : Date.parse(vm.value3[1]),
                adStatusParam: vm.editForm.adStatus == 'ON' ? '' : vm.radio1 == 0 ? 'ON':'W',
    ...
    mounted () {
      ...
      bus.$on('handleEditBox', function (message) {
        ...
        vm.imageUrl = message.detail.img2 ? message.detail.img2 : ''
         ...
         vm.radio1 = vm.editForm.adStatus == "W" ? '1':'0'
         vm.value3=[vm.editForm.onlineStart,vm.editForm.onlineEnd]
          // vm.editForm.onlineEnd = new Date(vm.editForm.onlineEnd)
        //  vm.editForm.img2 = this.editForm.img2
         if (vm.editForm.img2 == '--') {
           vm.editForm.img2 = process.env.API_ROOT+ root +this.editForm.img2
        }
