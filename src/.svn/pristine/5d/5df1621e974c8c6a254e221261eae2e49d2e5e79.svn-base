     ...
     <!-- 内容 -->
         <table class="table-details" width="100%">
          ...
          <tr>
           ...
           <tr>
              <td>创建时间：</td> 
            <td>{{tableData.gmtCreate}}</td>
             <td >下线时间：</td>
            <td>{{tableData.onlineEnd}}</td>
            </tr>
          <tr>
             <td>小图像：</td>
             <td><img :src="tableData.img2"  alt="--"></td>
             <td>banner位图像：</td>
             <td><img :src="tableData.img1" alt="--"></td>
          </tr>
        </table>
...
<script>
  import { merchantAdDtai,clickLog,root } from '../../api/api'
    ...
    methods: {
            ...
            merchantAdDtai(adId).then(data => {
                if (data.data.code === 200) {
                    // vm.tableData = data.data.data.dtail
                     vm.tableData = global.replaceObject(data.data.data.dtail, '--')
                   this.allData[0] =  data.data.data.totalClick
                    ...
                    vm.listClick[0] =  data.data.data.listClick
                     vm.listClick[0].adName =  data.data.data.dtail.adName
                      ...
                      vm.tableData.img1 =vm.tableData.img1 =='--'? "":`${process.env.IMG_SERVER}` + vm.tableData.img1;
                      vm.tableData.img2 = vm.tableData.img2 == '--' ? "":`${process.env.IMG_SERVER}` + vm.tableData.img2;
                       
                      vm.tableData.gmtCreate  = vm.tableData.gmtCreate  == '--' ? vm.tableData.gmtCreate :global.formatTime(vm.tableData.gmtCreate )
                        
                      vm.tableData.onlineEnd  = vm.tableData.onlineEnd  == '--' ? vm.tableData.onlineEnd :global.formatTime(vm.tableData.onlineEnd )
        ...
        getInTimeCount( start,end,type){
            if(!start && !end){
               switch (type) {
                    case 1:
                        this.banData[0].totalClickNum ="暂无数据"
                        break;
                    case 2:
                        this.listData[0].totalClickNum = "暂无数据"
                        break;
                    default:
                      this.timeData[0].totalClickNum = "暂无数据"
                      break;
                }
              return
            }
            let parem = {
               ...
               clickEndtTime:Date.parse(end)+(24*60*60-1)*1000,
               positionType:type
             }
