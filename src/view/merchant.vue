  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="150">
        <template scope="scope">
          ...
          <el-button size="small" type="danger" @click="handleDelete(scope.$index, scope.row)" v-if="deleteShow">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="新增" v-model="addFormVisible" :close-on-click-modal="false">
      <el-form :model="addForm" label-width="120px" :rules="addFormRules" ref="addForm">
        <el-form-item label="广告商名称" prop="comment">
...
<script>
  import { merchantList, addMerchant, modifyMerchant, deleteMerchant } from '../api/api'
  ...
  export default {
    ...
    mounted () {
      ...
      this.deleteShow = global.judgePermissionName('merchant:delete', this.$store.getters.getPermissionName)
    },
    computed: {
      // searchData () {
    ...
    methods: {
      ...
      handleDelete (index, row) {
        let vm = this
        vm.$confirm('确定要删除该广告商吗？', '提示', {}).then(() => {
          deleteMerchant({id: row.id}).then(res => {
