         ...
         if (vm.radio1 == 1 && (this.value3 == '' || this.addForm.onlineTime== undefined)  ) {
          ...
          if (valid) {
            vm.$confirm('确认提交吗？', '提示', {}).then(() => {
              ...
              // if (typeof vm.addForm.onlineEnd == 'number') {
              //   params.onlineEnd = vm.radio1 == 0 ? vm.addForm.onlineEnd : Date.parse(vm.addForm.onlineTime[1])
              // } else {
              //   params.onlineEnd = vm.radio1 == 0 ? Date.parse(vm.addForm.onlineEnd) : Date.parse(vm.addForm.onlineTime[1])
              // }
